...
export const bloggerApi = createApi({
  ...
  endpoints: (builder) => ({
    fetchBlogger: builder.query<BloggerBigType, string | undefined>({
      async queryFn(id) {
        if (!id) return { error: 'undefined id' };
        let blogger: any[] = [];
        try {
          const q = query(collection(db, 'bloggers'), where('id', '==', id));
    ...
    fetchBloggerListByAudience: builder.query<BloggerBigType[], string>({
      async queryFn(audienceType) {
        ...
        try {
          ...
          const querySnapshot = await getDocs(q);
          querySnapshot.forEach((doc) => bloggersList.push(doc.data()));
